...
html_filepaths=$(find ./dist/apps/company-app/exported/ -name "*.html" ! -path "*/index.html")
for filepath in $html_filepaths; do
  mv $filepath ${filepath%\.html}
done

aws s3 sync ./dist/apps/company-app/exported/ s3://$bucket_name/ --delete --acl public-read

# S3 上のオブジェクトに対し Content-Type を付与する
for filepath in $html_filepaths; do
  path=${filepath#\.\/dist/apps/company-app/exported\/}
  key=${path%\.html}
...
empty_index_html_directory=./dist/apps/company-app/exported/tmp/
...
for index_html_path in $target_index_html_paths; do
  key=${index_html_path#\.\/dist/apps/company-app/exported\/}
