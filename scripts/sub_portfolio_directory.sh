...
html_filepaths=$(find ./apps/portfolio-app/dist/ -name "*.html" ! -path "*/index.html")
for filepath in $html_filepaths; do
  mv $filepath ${filepath%\.html}
done

aws s3 sync ./apps/portfolio-app/dist/ s3://$bucket_name/ --delete --acl public-read

# S3 上のオブジェクトに対し Content-Type を付与する
for filepath in $html_filepaths; do
  path=${filepath#\.\/apps/portfolio-app/dist\/}
  key=${path%\.html}
...
empty_index_html_directory=./apps/portfolio-app/dist/tmp/
...
for index_html_path in $target_index_html_paths; do
  key=${index_html_path#\.\/apps/portfolio-app/dist\/}
