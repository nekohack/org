...
# astro build で出来る HTML から拡張子を取り除く
html_filepaths=$(find ./apps/portfolio-legacyapp/dist/ -name "*.html" ! -path "*/index.html")
for filepath in $html_filepaths; do
  mv $filepath ${filepath%\.html}
done

aws s3 sync ./apps/portfolio-legacyapp/dist/ s3://$bucket_name/ --delete --acl public-read

# S3 上のオブジェクトに対し Content-Type を付与する
for filepath in $html_filepaths; do
  path=${filepath#\.\/apps/portfolio-legacyapp/dist\/}
  key=${path%\.html}
...
empty_index_html_directory=./apps/portfolio-legacyapp/dist/tmp/
...
for index_html_path in $target_index_html_paths; do
  key=${index_html_path#\.\/apps/portfolio-legacyapp/dist\/}
